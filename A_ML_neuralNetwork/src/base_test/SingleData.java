		...
		for(int k=0;k<1000;k++){//循环十次
			...
			for(int i=0;i<middleNumber;i++){
				...
				for(int j=0;j<inNumber;j++){
					results = results+inWeight[j][i]*exam[j];
				}
			...
			System.out.println(result);
			//计算输出层神经元的梯度项
			double outGradientTerm = result*(1-result)*(0.67-result);
			//计算隐层神经元梯度项
			...
			//隐层到输出层之间权重更新 
			for(int i=0;i<middleNumber;i++){
				outWeight[i] = outWeight[i] + 0.3*midGradientTerm[i]*MidValues[i];
				//System.out.println(outWeight[i]);
			}
			//输入层到隐层的权重更新
			for(int i=0;i<inWeight.length;i++){
				for(int j=0;j<inWeight[0].length;j++){//按行更新
					inWeight[j][i] = inWeight[j][i]+0.3*midGradientTerm[i]*exam[j];
